  ...
  <div class="main-content p-2 p-md-4">
    ...
    <div v-else>
      ...
      <b-row>
        ...
        <b-col md="3" sm="6" class="mb-3">
          <StatTile icon="i-File-Horizontal-Text" :label="$t('Lines')" :value="num(kpis.lines_count)" theme="teal" />
        </b-col>
        <b-col md="3" sm="6" class="mb-3">
          <StatTile icon="i-Box1" :label="$t('QtyMoved')" :value="formatQty(kpis.qty_sum)" theme="indigo" />
        </b-col>
        <b-col md="3" sm="6" class="mb-3">
      ...
      <b-row>
        <b-col md="8" class="mb-3">
          <b-card class="shadow-soft border-0">
            ...
            <v-chart :options="chartTimeOptions" autoresize style="height:300px;" />
          </b-card>
        </b-col>
        ...
        <b-col md="4" class="mb-3">
          <b-card class="shadow-soft border-0">
            ...
            <v-chart :options="chartRouteOptions" autoresize style="height:300px;" />
          </b-card>
        </b-col>
      </b-row>
...
// ECharts modules already loaded in your app (pie, bar, line, tooltip, legend)
...
export default {
  ...
  components: { "v-chart": ECharts, "date-range-picker": DateRangePicker, StatTile },
  data() {
  ...
  computed: {
    ...
    chartTimeOptions(){
      ...
      const qty   = this.timeseries.map(x => Number(x.qty || 0));
      const val   = this.timeseries.map(x => Number(x.val || 0));

      return {
        tooltip: { trigger:'axis' },
        legend:  { data:[this.$t('Qty'), this.$t('Value')] },
        grid: { left: 10, right: 10, bottom: 10, top: 40, containLabel: true },
        xAxis: [{ type:'category', data: dates, axisTick:{show:false} }],
        yAxis: [
          { type:'value', name:this.$t('Qty') },
          { type:'value', name:this.$t('Value'), axisLabel:{ formatter: v => this.shortMoney(v) } }
        ],
        series: [
          { name:this.$t('Qty'), type:'line', smooth:true, data: qty },
          { name:this.$t('Value'), type:'bar', yAxisIndex:1, data: val }
        ]
      };
    },
    chartRouteOptions(){
      const top = this.routes.slice(0,7);
      const cats = top.map(r => `${r.from_name} → ${r.to_name}`);
      const vals = top.map(r => Number(r.val || 0));
      return {
        tooltip: { trigger:'axis', axisPointer:{ type:'shadow' }, formatter: (p)=> {
          const x = p[0]; return `${x.axisValue}<br/>${x.marker} ${this.$t('Value')}: <b>${this.money(x.value)}</b>`;
        }},
        grid: { left: 10, right: 10, bottom: 10, top: 10, containLabel: true },
        xAxis: { type:'value', axisLabel:{ formatter:v=>this.shortMoney(v) } },
        yAxis: { type:'category', data: cats, axisLabel:{ interval:0 } },
        series: [{ name:this.$t('Value'), type:'bar', data: vals }]
      };
    }
  },
  methods: {
    ...
    money(v){
      ...
      catch(e){ return `${this.currency} ${this.num(v).toLocaleString()}`; }
    },
    ...
    async exportPDF() {
      try {
        ...
        doc.setFontSize(16);
        const title = this.$t("Stock_Transfer_Report");
        ...
        y = (doc.autoTable.previous && doc.autoTable.previous.finalY) ? (doc.autoTable.previous.finalY + 12) : (y + 12);
        ...
        autoTable(doc, {
          ...
          headStyles: { font: "Vazirmatn", fontStyle: "bold", fillColor: [245, 245, 245], textColor: 20, halign: rtl ? "right" : "left" },
          columnStyles: {
            4: { halign: "right" }, // Qty
