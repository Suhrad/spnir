  ...
  <div class="main-content p-2 p-md-4">
    ...
    <div v-else>
      ...
      <b-row>
        ...
        <b-col md="3" sm="6" class="mb-3">
          <StatTile icon="i-File-Horizontal-Text" :label="$t('Purchases')" :value="num(kpis.total_purchases)" theme="teal"/>
        </b-col>
        <b-col md="3" sm="6" class="mb-3">
          <StatTile icon="i-Box1" :label="$t('QtyPurchased')" :value="formatQty(kpis.total_qty)" theme="indigo"/>
        </b-col>
        <b-col md="3" sm="6" class="mb-3">
      ...
      <b-row>
        <b-col md="8" class="mb-3">
          <b-card class="shadow-soft border-0">
            ...
            <v-chart :options="chartValueOptions" autoresize style="height:320px;" />
          </b-card>
        </b-col>
        <b-col md="4" class="mb-3">
          <b-card class="shadow-soft border-0">
            ...
            <v-chart :options="chartQtyOptions" autoresize style="height:320px;" />
          </b-card>
        </b-col>
      </b-row>
...
export default {
  ...
  computed:{
    ...
    chartValueOptions(){
      const cats = this.topByValue.map(x=>x.supplier);
      const vals = this.topByValue.map(x=>Number(x.value_sum||0));
      return {
        tooltip:{trigger:'axis', axisPointer:{type:'shadow'}, formatter:(p)=>`${p[0].axisValue}<br/>${p[0].marker} ${this.$t('TotalSpend')}: <b>${this.money(p[0].value)}</b>`},
        grid:{left:10,right:10,bottom:10,top:20,containLabel:true},
        xAxis:{type:'category', data:cats, axisLabel:{interval:0, rotate:0}},
        yAxis:{type:'value', axisLabel:{formatter:(v)=>this.shortMoney(v)}},
        series:[{type:'bar', data:vals}]
      };
    },
    chartQtyOptions(){
      const cats = this.topByQty.map(x=>x.supplier);
      const vals = this.topByQty.map(x=>Number(x.qty_sum||0));
      return {
  ...
  methods:{
    ...
    money(v){
      ...
      catch(e){ return `${this.currency} ${Number(v||0).toLocaleString()}`; }
    },
    ...
    async exportPDF(){
      try{
        ...
        const title = this.$t('Top_Suppliers_Report');
        ...
        doc.autoTable({
          ...
          headStyles: { font: "Vazirmatn", fontStyle: "bold", fillColor: [11,95,255], textColor: 255, halign: rtl ? 'right' : 'left' },
