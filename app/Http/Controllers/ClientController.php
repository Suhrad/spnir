...
class ClientController extends BaseController
{
    ...
    {
        $addressLine1 = $request->input('address_line_1', $request->input('adresse'));
        $addressLine2 = $request->input('address_line_2');
        $addressLine3 = $request->input('address_line_3');

        return [
            'business_company_id' => $request->input('business_company_id') ?: $this->defaultBusinessCompanyId(),
            'name' => $request->input('name'),
            'legal_name' => $request->input('legal_name', $request->input('name')),
            'trade_name' => $request->input('trade_name', $request->input('name')),
            'adresse' => $request->input('adresse', implode("\n", array_filter([$addressLine1, $addressLine2, $addressLine3]))),
            ...
            'tax_number' => $request->input('tax_number', $request->input('gstin', $request->input('pan'))),
    ...
    protected function defaultBusinessCompanyId(): int
    {
        return (int) BusinessCompany::whereNull('deleted_at')->orderBy('id')->value('id');
    }
    ...
    public function index(request $request)
    {
        ...
        $data = array();
        $clients = Client::with('company')->where('deleted_at', '=', null);

        if ($request->filled('business_company_id')) {
            $clients->where('business_company_id', $request->business_company_id);
        }
        ...
        foreach ($clients as $client) {
            ...
            $item['legal_name'] = $client->legal_name;
            $item['trade_name'] = $client->trade_name;
            $item['phone'] = $client->phone;
            $item['tax_number'] = $client->tax_number;
            $item['gstin'] = $client->gstin;
            $item['pan'] = $client->pan;
            ...
            $item['pin_code'] = $client->pin_code;
            $item['adresse'] = $client->adresse;
            $item['address_line_1'] = $client->address_line_1;
            $item['address_line_2'] = $client->address_line_2;
            $item['address_line_3'] = $client->address_line_3;
            $item['place_of_supply'] = $client->place_of_supply;
            $item['gst_type'] = $client->gst_type;
            $item['contact_person'] = $client->contact_person;
            $item['business_company_id'] = $client->business_company_id;
            $item['company_name'] = optional($client->company)->name;
            $item['is_royalty_eligible'] = $client->is_royalty_eligible;
            $item['points'] = $client->points;
            $item['opening_balance'] = $client->opening_balance;
            $item['opening_balance_type'] = $client->opening_balance_type;
            $data[] = $item;
        }
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            'name' => 'required',
            'business_company_id' => 'nullable|integer|exists:business_companies,id',
            ]
        );
        ...
        if($request['is_royalty_eligible'] == '1' || $request['is_royalty_eligible'] == 'true'){
         ...
         $client = Client::create($this->customerPayload($request) + [
    ...
    public function Get_Clients_Without_Paginate()
    {
        $clients = Client::where('deleted_at', '=', null)
            ->when(request('business_company_id'), function ($query) {
