  ...
  methods: {
    ...
    onGridProductChange: function onGridProductChange(detail) {
      ...
      if (product) {
        ...
        axios.get("/show_product_data/" + product.id + "/" + (product.product_variant_id || "null")).then(function (response) {
          ...
          _this5.fetchPriceHistory(detail);
        });
      }
    },
    fetchPriceHistory: function fetchPriceHistory(detail) {
      var _this6 = this;
      ...
      axios.get("sales/party_product_history?client_id=".concat(this.sale.client_id, "&product_id=").concat(detail.product_id)).then(function (res) {
        _this6.historyState.list = res.data.history || [];
        _this6.historyState.averagePrice = res.data.average_price || 0;
        _this6.historyState.lastPrice = _this6.historyState.list[0] ? _this6.historyState.list[0].price : null;
        _this6.historyState.isLoading = false;
      })["catch"](function () {
        _this6.historyState.isLoading = false;
      });
    },
    applyHistoryPrice: function applyHistoryPrice(price) {
      var _this7 = this;
      var detail = this.details.find(function (d) {
        return d.detail_id === _this7.historyState.activeDetailId;
      });
      if (detail) {
      ...
      if (this.sale.warehouse_id != "" && this.sale.warehouse_id != null) {
        this.timer = setTimeout(function () {
          ...
          if (barcode.length === 13 && !isNaN(barcode)) {
            ...
            _this8.search_input = '';
            if (_this8.$refs.product_autocomplete) {
    ...
    Get_Product_Details: function Get_Product_Details(product_id, variant_id) {
      ...
      axios.get("/show_product_data/" + product_id + "/" + variant_id).then(function (response) {
        ...
        _this1.product.product_type = response.data.product_type;
        _this1.product.Net_price = response.data.Net_price;
        _this1.product.Unit_price = response.data.Unit_price;
        _this1.product.taxe = response.data.tax_price;
        _this1.product.tax_method = response.data.tax_method;
        _this1.product.tax_percent = response.data.tax_percent;
        _this1.product.unitSale = response.data.unitSale;
        _this1.product.sale_unit_id = response.data.sale_unit_id;
    ...
    GetElements: function GetElements() {
      ...
      axios.get("sales/create").then(function (response) {
        ...
        _this10.transporters = response.data.transporters;
        _this10.point_to_amount_rate = response.data.point_to_amount_rate;
    ...
    updateNote: function updateNote() {
      ...
      var warehouse = this.warehouses.find(function (w) {
        return w.id === _this11.sale.warehouse_id;
      });
      var symbol = "NP";
